...
export default function HomePage() {
  ...
  if (authLoading) return <div className="min-h-screen bg-cream flex items-center justify-center dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><div className="w-8 h-8 border-2 border-ruby border-t-transparent rounded-full animate-spin" /></div>;
  if (user) return null;
  return (<div className="min-h-screen bg-cream flex flex-col dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><ParticleBackground count={30} />
    ...
    <div className="relative flex-1 flex items-center justify-center px-4 py-12">
      <div className="max-w-6xl mx-auto w-full grid md:grid-cols-2 gap-12 items-center">
        <motion.div initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} className="text-center md:text-right order-2 md:order-1">
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3, type: 'spring' }} className="inline-flex items-center justify-center mb-6"><div className="relative"><YaqutIcon size={80} animate /><SparkleEffect count={8} /></div></motion.div>
          <h1 className="text-4xl md:text-6xl font-black text-navy mb-4 leading-tight dark:text-cream">یاقوت<br /><span className="text-ruby">سمینار</span></h1>
          <div className="flex items-center justify-center md:justify-start gap-8">{[{ icon: Trophy, label: 'مسابقه', value: '۶ تیم' }, { icon: Gem, label: 'یاقوت', value: '۱۴۲' }, { icon: Users, label: 'شرکت‌کننده', value: '۱۵ نفر' }].map((s, i) => <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }} className="text-center"><s.icon className="w-5 h-5 text-ruby mb-1 mx-auto" /><div className="text-lg font-bold text-navy dark:text-cream">{s.value}</div><div className="text-xs text-sky">{s.label}</div></motion.div>)}</div>
        </motion.div>
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8, delay: 0.2 }} className="order-1 md:order-2">
          <div className="w-full max-w-md mx-auto"><div className="relative bg-white/70 backdrop-blur-xl rounded-3xl border border-navy/10 p-8 shadow-2xl dark:bg-navy/70 dark:border-beige/15"><div className="relative">
    ...
    <footer className="relative z-10 border-t border-navy/8 py-6 dark:border-beige/10"><div className="max-w-7xl mx-auto px-4 text-center text-sm text-sky">مدرسه راهنمایی علامه حلی ۱ تهران — یاقوت سمینار ۱۴۰۵</div></footer>
  </div>);
}
