...
export default function PublicProjectPage({ params }: { params: { id: string } }) {
  ...
  if (error || !project) return <div className="min-h-screen bg-[#669BBC] flex items-center justify-center dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><Card className="p-6 sm:p-8 text-center"><p className="text-red-500">{error || 'پروژه یافت نشد'}</p><Link href="/leaderboard" className="mt-4 inline-block text-sky hover:text-ruby">بازگشت به رتبه‌بندی</Link></Card></div>;
