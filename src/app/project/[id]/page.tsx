...
export default function PublicProjectPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    // Fetch project via leaderboard to get public data
    leaderboardAPI.get().then((lb) => {
      const entry = lb.leaderboard.find((e) => e.id === params.id);
      if (entry) {
        setRank(entry.rank);
        // Fetch full project details
        return projectsAPI.getPublic(params.id);
      }
      throw new Error('پروژه یافت نشد');
    }).then((data) => {
      setProject(data.project);
    }).catch((err) => {
      setError(err.message || 'پروژه یافت نشد');
  ...
  if (error || !project) return <div className="min-h-screen bg-cream flex items-center justify-center dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><Card className="p-8 text-center"><p className="text-ruby-glow">{error || 'پروژه یافت نشد'}</p><Link href="/leaderboard" className="mt-4 inline-block text-sky hover:text-ruby">بازگشت به رتبه‌بندی</Link></Card></div>;
  ...
  return (<div className="min-h-screen bg-cream dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><ParticleBackground count={15} /><Navbar />
    <main className="relative z-10 max-w-2xl mx-auto px-4 pt-24 pb-12"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      <Card className="p-6"><h2 className="text-lg font-bold text-navy mb-4 flex items-center gap-2 dark:text-cream"><Users className="w-5 h-5 text-sky" />اعضای تیم</h2><div className="space-y-3">{project.members?.map((m, i) => <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-center gap-3 p-3 rounded-xl bg-navy/3 dark:bg-navy-light/30"><div className="w-10 h-10 rounded-full bg-gradient-to-br from-ruby to-beige flex items-center justify-center text-cream font-bold text-sm">{m.name.charAt(0)}</div><div><div className="font-medium text-navy dark:text-cream">{m.name}</div><div className="text-xs text-sky">{m.period && 'دوره: ' + m.period}</div></div></motion.div>)}</div></Card>
