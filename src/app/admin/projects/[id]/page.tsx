...
export default function AdminProjectDetail() {
  ...
  return (<div className="min-h-screen bg-cream dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><Navbar /><main className="relative z-10 max-w-2xl mx-auto px-4 pt-24 pb-12"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
    <div className="relative text-center">
      ...
      {project.description && <p className="text-navy/60 dark:text-beige-light">{project.description}</p>}
    </div>
    <Card className="p-8 text-center relative overflow-hidden"><SparkleEffect count={10} /><motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', bounce: 0.4 }}><YaqutIcon size={64} animate /></motion.div><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}><div className="text-6xl font-black text-navy mt-4 dark:text-cream">{toPersianNumber(project.yaqut_count)}</div><div className="text-lg text-navy/50 mt-2 dark:text-beige-light">یاقوت</div></motion.div></Card>
    <Card className="p-6"><h2 className="text-lg font-bold text-navy mb-4 flex items-center gap-2 dark:text-cream"><Users className="w-5 h-5 text-sky" />اعضای تیم</h2><div className="space-y-3">{project.members?.map((m, i) => <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }} className="flex items-center gap-3 p-3 rounded-xl bg-navy/3 dark:bg-navy-light/30"><div className="w-10 h-10 rounded-full bg-gradient-to-br from-ruby to-beige flex items-center justify-center text-cream font-bold">{m.name.charAt(0)}</div><div><div className="font-medium text-navy dark:text-cream">{m.name}</div><div className="text-xs text-sky">{m.period && 'دوره: ' + m.period}{m.student_id && ' • کد: ' + m.student_id}</div></div></motion.div>)}</div></Card>
