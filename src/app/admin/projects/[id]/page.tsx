...
export default function AdminProjectDetail() {
  ...
  if (loading) return <div className="min-h-screen bg-[#669BBC] flex items-center justify-center dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><div className="w-8 h-8 border-2 border-ruby border-t-transparent rounded-full animate-spin" /></div>;
  if (error || !project) return <div className="min-h-screen bg-[#669BBC] flex items-center justify-center dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><Card className="p-6 sm:p-8 text-center"><p className="text-red-500">{error || 'پروژه یافت نشد'}</p></Card></div>;
  return (<div className="min-h-screen bg-[#669BBC] dark:bg-gradient-to-br dark:from-navy-dark dark:via-navy dark:to-navy-dark"><Navbar /><main className="relative z-10 max-w-2xl mx-auto px-4 pt-24 pb-12"><motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-4 sm:space-y-6">
