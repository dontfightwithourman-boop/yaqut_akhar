...
export default function AwardPage() {
  ...
  return (<div className="space-y-4 sm:space-y-6">
    <div className="relative"><h1 className="text-xl sm:text-2xl font-bold text-navy mb-2 dark:text-cream">اعطای یاقوت</h1><p className="text-sm text-navy/50 dark:text-beige-light">یاقوت به پروژه‌های برتر اعطا کنید</p>
      <AnimatePresence>{showConfetti && <div className="fixed inset-0 pointer-events-none z-50"><YaqutConfetti trigger={showConfetti} />{lastAwarded && <motion.div initial={{ opacity: 0, scale: 0.8, y: 50 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8 }} className="fixed top-1/3 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-xl rounded-3xl border border-ruby/20 p-6 sm:p-8 shadow-2xl shadow-ruby/10 text-center dark:bg-navy/95 dark:border-ruby/30 dark:shadow-ruby/20"><YaqutIcon size={48} animate /><h3 className="text-lg sm:text-xl font-bold text-navy mt-4 dark:text-cream">یاقوت اعطا شد!</h3><p className="text-navy/50 mt-2 dark:text-beige-light">{toPersianNumber(lastAwarded.amount)} یاقوت به {lastAwarded.projectName}</p></motion.div>}</div>}</AnimatePresence>
    </div>
    <Card className="p-4 sm:p-6"><div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4"><Input label="تعداد یاقوت" type="number" min="1" placeholder="10" value={amount} onChange={(e) => setAmount(e.target.value)} /><Input label="یادداشت" placeholder="ارائه، پوستر، ..." value={note} onChange={(e) => setNote(e.target.value)} /><div className="flex items-end"><Button onClick={handleAward} loading={awarding} disabled={selected.length === 0 || !amount || parseInt(amount) < 1} className="w-full"><Gem className="w-4 h-4" />اعطای {amount ? toPersianNumber(parseInt(amount)) : ''} یاقوت</Button></div></div>
      <div className="flex items-center justify-between"><p className="text-xs sm:text-sm text-navy/50 dark:text-beige-light">{toPersianNumber(selected.length)} پروژه انتخاب شده</p><button onClick={selectAll} className="text-xs sm:text-sm text-ruby hover:text-ruby-glow transition-colors">{selected.length === projects.length ? 'انتخاب هیچکدام' : 'انتخاب همه'}</button></div></Card>
