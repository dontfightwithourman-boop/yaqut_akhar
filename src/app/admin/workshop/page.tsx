...
export default function WorkshopPage() {
  ...
  const fetchData = async () => {
    try {
      const [itemsRes, loansRes] = await Promise.all([workshopAPI.getItems(), workshopAPI.getLoans()]);
  ...
  const handleItemSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) { await workshopAPI.updateItem(editingItem.id, { name: iName, location: iLoc, quantity: parseInt(iQty) || 1, description: iDesc }); }
      else { await workshopAPI.createItem({ name: iName, location: iLoc, quantity: parseInt(iQty) || 1, description: iDesc }); }
  ...
  const handleDeleteItem = async (id: string) => {
    if (!confirm('آیا از حذف این وسیله مطمئن هستید؟')) return;
    try { await workshopAPI.deleteItem(id); fetchData(); } catch { /* */ }
  };
  ...
  const handleLoanSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await workshopAPI.createLoan({ item_id: lItemId, item_name: lItemName, quantity: parseInt(lQty) || 1, group_name: lGroup, borrower_name: lBorrower, borrow_date: lBorrowDate, return_date: lReturnDate });
  ...
  const handleReturnLoan = async (id: string) => {
    try { await workshopAPI.updateLoan(id, { status: 'returned' }); fetchData(); } catch { /* */ }
  };
  const handleDeleteLoan = async (id: string) => {
    if (!confirm('آیا از حذف این قرض مطمئن هستید؟')) return;
    try { await workshopAPI.deleteLoan(id); fetchData(); } catch { /* */ }
  };

  // Sort loans by return date (closest deadline first)
  ...
  const getLoanStatus = (loan: WorkshopLoan) => {
    if (loan.status === 'returned') return 'returned';
    if (loan.return_date < today) return 'overdue';
    return 'active';
  };
          ...
          <Card key={item.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1"><h3 className="font-bold text-navy text-sm dark:text-cream">{item.name}</h3><span className="text-xs px-2 py-0.5 rounded-full bg-sky/10 text-sky dark:bg-sky/20">{toPersianNumber(item.quantity)} عدد</span></div>
              ...
              <div className="flex gap-1 shrink-0">
                <button onClick={() => openEditItem(item)} className="p-2 rounded-lg hover:bg-navy/5 text-navy/40 hover:text-navy transition-all dark:hover:bg-navy-light/30 dark:hover:text-cream"><Edit className="w-4 h-4" /></button>
                <button onClick={() => handleDeleteItem(item.id)} className="p-2 rounded-lg hover:bg-ruby/5 text-navy/40 hover:text-ruby transition-all dark:hover:bg-ruby/10 dark:hover:text-ruby-glow"><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          </Card>
        ))}</div>}
    ...
    {tab === 'loans' && (
      <div className="space-y-4">
        ...
        <div className="space-y-3">{sortedLoans.map((loan) => {
          ...
          return (
            <Card key={loan.id} className={`p-4 ${status === 'overdue' ? 'ring-2 ring-red-400/50 dark:ring-red-500/50' : ''}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="font-bold text-navy text-sm dark:text-cream">{loan.item_name}</h3>
                    {status === 'overdue' && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-red-500/10 text-red-500 dark:bg-red-500/20"><AlertTriangle className="w-3 h-3" />سررسید گذشته</span>}
                  ...
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-navy/50 dark:text-beige-light">
                    ...
                    {loan.group_name && <span>کلاس: {loan.group_name}</span>}
                    {loan.borrower_name && <span>قرض‌گیرنده: {loan.borrower_name}</span>}
                    <span>تاریخ قرض: {loan.borrow_date}</span>
                    <span>مهلت بازگشت: {loan.return_date}</span>
                  </div>
                </div>
                <div className="flex gap-1 shrink-0">
                  {status !== 'returned' && <button onClick={() => handleReturnLoan(loan.id)} className="p-2 rounded-lg hover:bg-emerald-500/10 text-navy/40 hover:text-emerald-600 transition-all dark:hover:bg-emerald-500/20 dark:hover:text-emerald-400" title="بازگشت"><CheckCircle className="w-4 h-4" /></button>}
                  <button onClick={() => handleDeleteLoan(loan.id)} className="p-2 rounded-lg hover:bg-ruby/5 text-navy/40 hover:text-ruby transition-all dark:hover:bg-ruby/10 dark:hover:text-ruby-glow"><Trash2 className="w-4 h-4" /></button>
                </div>
              </div>
            </Card>
          );
    ...
    <Modal isOpen={showLoanModal} onClose={() => setShowLoanModal(false)} title="قرض جدید">
      <form onSubmit={handleLoanSubmit} className="space-y-4">
        <div className="space-y-1.5"><label className="block text-sm font-medium text-navy dark:text-beige-light">انتخاب وسیله</label>
          <select value={lItemId} onChange={(e) => { const item = items.find((i) => i.id === e.target.value); setLItemId(e.target.value); setLItemName(item?.name || ''); }} className="w-full px-4 py-3 rounded-xl bg-white/80 border border-navy/15 text-navy focus:outline-none focus:ring-2 focus:ring-ruby/50 dark:bg-navy-light/40 dark:border-beige/15 dark:text-cream" dir="auto">
            <option value="">انتخاب کنید...</option>
            {items.map((item) => <option key={item.id} value={item.id}>{item.name} ({toPersianNumber(item.quantity)} عدد)</option>)}
          </select>
        </div>
        <Input label="تعداد قرض" type="number" min="1" value={lQty} onChange={(e) => setLQty(e.target.value)} />
        <Input label="نام قرض‌گیرنده" value={lBorrower} onChange={(e) => setLBorrower(e.target.value)} />
        <Input label="کلاس / گروه" value={lGroup} onChange={(e) => setLGroup(e.target.value)} />
