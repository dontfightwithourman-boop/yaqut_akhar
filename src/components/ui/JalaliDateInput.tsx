...
export function toJalaliDateShort(dateStr: string): string {
  if (!dateStr) return '';
  try {
    const d = new Date(dateStr);
    if (isNaN(d.getTime())) return dateStr;
...
export function getJalaliString(jalaliStr: string): string {
  if (!jalaliStr) return '';
  ...
  if (monthNum >= 1 && monthNum <= 12) {
    const months = ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'];
    return `${toPersianNumber(parts[0])}/${months[monthNum - 1]}/${toPersianNumber(parts[2])}`;
  }
  return jalaliStr;
}
