...
const Input = forwardRef<HTMLInputElement, InputProps>(({ label, error, icon, className = '', ...props }, ref) => (
  <div className="space-y-1.5">
    ...
    <div className="relative">
      ...
      <input ref={ref} className={`w-full px-4 py-3 rounded-xl bg-white/80 border border-navy/15 text-navy placeholder-navy/30 focus:outline-none focus:ring-2 focus:ring-pearl/50 focus:border-pearl/50 transition-all duration-200 dark:bg-navy-light/40 dark:border-beige/15 dark:text-cream dark:placeholder-sky/40 ${icon ? 'pr-10' : ''} ${error ? 'border-red-500/50' : ''} ${className}`} dir="auto" {...props} />
    </div>
