  ...
  return <div className="space-y-3">{entries.map((e, i) => (
    <Link key={e.id} href={`/project/${e.id}`}>
      <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
        className={`flex items-center gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl bg-white/60 backdrop-blur-sm border border-sky/15 hover:bg-white/80 hover:shadow-lg hover:shadow-navy/5 transition-all duration-300 dark:bg-navy/60 dark:border-beige/10 dark:hover:bg-navy-light/30 dark:hover:shadow-ruby/5 cursor-pointer ${e.rank === 1 ? 'bg-gradient-to-r from-beige/8 to-transparent border-beige/15 dark:from-beige/10 dark:border-beige/20' : ''} ${e.rank === 2 ? 'bg-gradient-to-r from-sky/8 to-transparent border-sky/15 dark:from-sky/10 dark:border-sky/20' : ''} ${e.rank === 3 ? 'bg-gradient-to-r from-ruby/5 to-transparent border-ruby/10 dark:from-ruby/10 dark:border-ruby/20' : ''}`}>
        ...
        {e.logo ? (
          ...
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-sky/20 to-beige/20 flex items-center justify-center border border-sky/15 dark:border-beige/20 shrink-0">
