...
export default function RankBadge({ rank, size = 'md' }: { rank: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  if (rank === 3) return <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className={`${s[size]} rounded-full bg-gradient-to-br from-pearl-glow to-pearl flex items-center justify-center shadow-lg shadow-pearl/30`}><span className={`${is[size]} font-black text-navy`}>۳</span></motion.div>;
